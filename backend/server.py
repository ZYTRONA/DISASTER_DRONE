...
PRIORITY_ORDER = {'Critical': 0, 'Urgent': 1, 'High': 2, 'Normal': 3}
PRIORITY_ALIASES = {
    'critical': 'Critical',
    'sos': 'Critical',
    'emergency': 'Critical',
...
CORS(app, origins='*', supports_credentials=True)  # Allow all origins for public access

socketio = SocketIO(
    app,
    cors_allowed_origins='*',  # Allow all origins for public access
    ping_timeout=120,  # Increase timeout for slow connections
...
def init_indexes():
    ...
    db.requests.create_index([('priority', ASCENDING)])
...
def serialize_doc(doc) -> dict:
    ...
    if '_id' in doc_copy:
        ...
        doc_copy['id'] = doc_copy['_id']  # Add id field for frontend compatibility
...
def triage_sort_key(r):
    ...
    priority_rank = PRIORITY_ORDER.get(normalize_priority(r.get('priority')), 99)
    created = r.get('created_at') or ''
    return (priority_rank, created)


def normalize_priority(value):
    """Normalize mobile urgency input without changing request workflow status."""
    if value is None:
        return 'Normal'
    key = str(value).strip().lower()
    return PRIORITY_ALIASES.get(key, 'Normal')
...
def get_stats():
    ...
    try:
        ...
        urgent = requests_col.count_documents({'priority': 'Urgent'})
        high = requests_col.count_documents({'priority': 'High'})
...
def submit_request():
    ...
    priority_input = data.get('priority')
    urgency = normalize_priority(priority_input)
    requested_priority = None
    if priority_input is not None:
        requested_priority = str(priority_input).strip() or None
    status = 'Pending'
    ...
    # NEW: Extract items list from cart or payload
    items_list = []
    if isinstance(cart, dict) and 'items_list' in cart:
        items_list = cart.get('items_list') or []
    elif isinstance(data.get('items'), list):
        items_list = data.get('items') or []
    
    # NEW: Log what we're receiving
    logger.info('Request: resource=%s, items=%s, note=%s', resource, items_list, note[:100])
    ...
    try:
        ...
        request_doc = {
            ...
            'lat': lat,
            'lon': lon,
            'items': items_list,  # NEW: Store items separately
            'priority': urgency,
        ...
        logger.info('New request %s (%s) priority=%s', ref_id, resource, urgency)
